...
New account registration is the wallet creation process. Birdchain wallet has three available types of tokens in it. The first one is the application token itself, the second one is ERC223 BIRD token represented as a cryptocurrency and available on different crypto exchanges. The third one is Ethereum crypto - added into Birdchain wallet to provide a possibility to the user to exchange both in-chain token, BIRD token onto ETH manually or automatically during mining process or later after token will be received by the user. Also it’s necessary to provide the ability to exchange singular in-chain token into available outside of the Birdchain network BIRD token.  
...
* **Balances Manager** - contains all functionality related to Birdchain wallets, also contains local cache of all balances inside the networking including in-chain token, BIRD public token and local ETH balances. Tightly coupled with in-memory Hot Storage.
